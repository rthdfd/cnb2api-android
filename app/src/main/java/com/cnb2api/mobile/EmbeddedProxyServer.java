...
public final class EmbeddedProxyServer {
    ...
    private void handleChat(Request request, OutputStream output) throws Exception {
        ...
        // True streaming (no tools): forward upstream SSE to client in real-time
        if (stream && !promptTools) {
            streamUpstreamToClient(output, upstreamRequest, model, started);
            return;
        }

        // For non-streaming or tools (need accumulation for XYML parsing):
    ...
    private void streamUpstreamToClient(OutputStream output, JSONObject request,
                            ...
                            if (line.startsWith("data:")) {
                                ...
                                try {
                                    ...
                                    if (choices != null && choices.length() > 0) {
                                        ...
                                        if (delta != null) {
                                            totalChars += delta.optString("content", "").length();
                                        }
                                    }
